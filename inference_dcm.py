...
def create_report(inference, header, orig_vol, pred_vol):
    ...
    draw.multiline_text((10, 90), f"Patient ID: {header.PatientID}", (255, 255, 255), font=main_font)
    ...
    pil_i = Image.fromarray(nd_img, mode="L").resize(400,400)
    ...
    pil_pred = Image.fromarray(nd_pred, mode="L").resize((400, 400))
...
def get_series_for_inference(path):
    ...
    for dicom in dicoms:
        if dicom.SeriesDescription == 'T1pre_reg':
...
if __name__ == "__main__":
    ...
    root_dir = os.getcwd()
    model_dir = os.path.join(root_dir,  'section3', 'src', 'model.pth')

    inference_agent = UNetInferenceAgent(
        device="cpu",
        parameter_file_path=model_dir)
