...
def LoadHippocampusData(root_dir, y_shape, z_shape):
    ...
    '''
    root_dir = os.getcwd()
    image_dir = os.path.join(root_dir,  'out', 'images')
    label_dir = os.path.join(root_dir, 'out','labels')
    ...
    for f in images:
        ...
        print('Min: %.3f, Max: %.3f' % (np.asarray(image).min(), np.asarray(image).max()))
        ...
        # ANSWER: Since our labels is not a binary map
